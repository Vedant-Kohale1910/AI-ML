...
class Engine:
    ...
    W_V2 = dict(skill=0.45,assess=0.25,exp=0.15,cert=0.10,edu=0.05)

    def __init__(self, variant="v1"):
        self.variant = variant
        self.W = self.W_V2 if variant=="v2" else self.W_V1
        self.VERSION = "v2.0-treatment" if variant=="v2" else "v1.3-control"
        self.students:Dict[int,Dict]={}; self.jobs:Dict[int,Dict]={}
    ...
    def load(self,sp,jp):
        ...
        for r in csv.DictReader(open(jp)):
            j=int(r["job_id"])
            self.jobs[j]={"id":j,"title":r["title"],"company":r["company"],"req":norm(r.get("required_skills",""))}
