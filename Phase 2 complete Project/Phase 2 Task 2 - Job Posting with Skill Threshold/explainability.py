...
def build_reason(student_name, job_role, t_result, m_result):
    ...
    if t_result["eligible"]:
        if len(passed) == 0:
            # eligible with nothing in the passed list (e.g. a role with no
            # hard skill thresholds) — don't index into an empty list
            parts.append(f"{student_name} meets the requirements for {job_role}.")
        elif len(passed) == 1:
            parts.append(f"{student_name} meets the minimum requirement for {job_role} ({passed[0]}).")
        else:
