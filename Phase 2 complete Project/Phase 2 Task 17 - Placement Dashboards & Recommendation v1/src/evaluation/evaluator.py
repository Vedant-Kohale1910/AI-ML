...
class Evaluator:
    ...
    def evaluate_system(self, students: List[Dict[str, Any]], 
        ...
        # Validate recommendations. batch_recommend returns {student_id: [recs]},
        # so carry the student_id from the dict key onto each flattened rec —
        # the individual rec payloads only carry job_id + score.
        all_recs_flat = []
        for student_id, recs in all_recommendations.items():
            for r in recs:
                all_recs_flat.append({**r, 'student_id': r.get('student_id', student_id)})
